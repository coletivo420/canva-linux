...
export function main(): number {
  ...
  const visibleMissingDescription = actions.filter((action) => (
    action.planned !== true
    && action.kind === 'command'
  ...
  if (visibleMissingDescription.length) {
    throw new Error(`Visible actions missing description: ${visibleMissingDescription.map((action) => action.id).join(', ')}`);
  }

  const shell = fs.readFileSync(path.join(rootDir, 'canva-linux.sh'), 'utf8');
  const ids = [...shell.matchAll(/run_action_by_id\s+["']([^"']+)["']/g)].map((match) => match[1]).filter(Boolean);
  const missing = ids.filter((id) => !actionIds.has(id));
  if (missing.length) {
    throw new Error(`Shell references unknown action IDs: ${[...new Set(missing)].join(', ')}`);
  }
...
if (require.main === module && /check-shell-action-ids\.js$/.test(process.argv[1] || '')) {
  try {
    process.exit(main());
  } catch (error) {
