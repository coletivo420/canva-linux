...
export type { ActionGroup, ActionKind, CanvaAction, InstallScope } from './action-types';
...
export function validateActions(actions: unknown): asserts actions is CanvaAction[] {
...
export function loadActions(rootDir = findProjectRoot()): CanvaAction[] {
  ...
  const actions = JSON.parse(fs.readFileSync(actionsPath(resolvedRoot), 'utf8')) as unknown;
...
export function formatActionCommand(action: CanvaAction | undefined | null): string {
  if (!action || !('command' in action) || !action.command) return '';
  return [action.command, ...(Array.isArray(action.args) ? action.args : [])].map((part) => shellQuote(String(part))).join(' ');
}

export function actionRequiresConfirmation(action: CanvaAction | undefined | null): boolean {
  return Boolean(action?.dangerous || action?.requiresConfirmation);
}

export function getActionsByGroup(group: ActionGroup, rootDir = findProjectRoot()): CanvaAction[] {
  return loadActions(rootDir).filter((action) => action.group === group && !action.hidden);
}

export function getActionById(id: string, rootDir = findProjectRoot()): CanvaAction | undefined {
  return loadActions(rootDir).find((action) => action.id === id);
}

export function getActionByCliFlag(flag: string, rootDir = findProjectRoot()): CanvaAction | undefined {
  return loadActions(rootDir).find((action) => Array.isArray(action.cli) && action.cli.includes(flag));
}
