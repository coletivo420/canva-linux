...
export function main(): number {
  ...
  for (const action of actions) {
    if (action.scope === "system" && action.requiresRoot !== true) {
      failures.push(
        `${action.id}: system-scope actions must set requiresRoot=true`,
      );
    }
    ...
    if (
      ...
      (("command" in action && action.command) ||
        ("args" in action && action.args))
    ) {
      failures.push(
        `${action.id}: planned actions must not define command/args`,
      );
    }
...
if (
  require.main === module &&
  /check-action-contract\.js$/.test(process.argv[1] || "")
) {
  try {
    process.exit(main());
  } catch (error) {
