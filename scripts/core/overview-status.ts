...
function readPackage(rootDir: string) {
  return JSON.parse(fs.readFileSync(path.join(rootDir, 'package.json'), 'utf8')) as { version?: string; repository?: { url?: string } };
}

function readPhase(rootDir: string) {
  const content = fs.readFileSync(path.join(rootDir, 'scripts/app-identity-common.sh'), 'utf8');
...
function detect(rootDir: string): DetectionLines {
  ...
  if ((result.status ?? 0) !== 0) {
    process.stderr.write(`[warn] Installation detection exited with status ${result.status ?? 'unknown'}.\n`);
  }
...
if (require.main === module && /overview-status\.js$/.test(process.argv[1] || '')) {
  try {
    process.exit(main());
  } catch (error) {
