...
export function main(): number {
  ...
  if (pkg.main !== expectedMain) {
    failures.push(`package.json main must be ${expectedMain}, got: ${pkg.main}`);
  }

  if (pkg.build?.extraMetadata?.main !== expectedMain) {
    failures.push('build.extraMetadata.main must match compiled runtime entrypoint');
  }
  ...
  const preloadBundlePath = path.join(rootDir, '.build/electron/preload/canva.bundle.js');
  ...
  if (fs.existsSync(compiledMainPath)) {
    ...
    if (compiledMain.includes("require('../../package.json')")) {
      failures.push('compiled main must not require ../../package.json from .build/');
    }
  }
...
if (require.main === module && /check-runtime-build\.js$/.test(process.argv[1] || '')) {
  try {
    process.exit(main());
  } catch (error) {
    console.error(`[runtime-build-check] ${error instanceof Error ? error.message : String(error)}`);
    process.exit(1);
  }
}
