...
print_display_backend_guidance(){ local run_cmd="$1"; ui_subsection "Display backend checks"; ui_cmd "${run_cmd} --force-wayland"; ui_cmd "${run_cmd} --force-x11"; }
print_gpu_backend_guidance(){ local run_cmd="$1"; ui_subsection "GPU backend checks"; ui_cmd "${run_cmd} --gpu-backend=auto"; ui_cmd "${run_cmd} --gpu-backend=opengl"; ui_cmd "${run_cmd} --gpu-backend=vulkan"; ui_cmd "${run_cmd} --gpu-backend=software"; }
