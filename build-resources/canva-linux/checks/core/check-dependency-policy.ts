...
export function main(): number {
  ...
  if (
    !fs.existsSync(
      path.join(rootDir, "build-resources/c420ui/scripts/electron-builder-before-build.ts"),
    )
  ) {
    console.error(
      "[dependency-policy] missing build-resources/c420ui/scripts/electron-builder-before-build.ts",
    );
    failed = true;
  }
