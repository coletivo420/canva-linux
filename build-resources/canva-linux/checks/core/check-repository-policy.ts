...
const requiredStandaloneEntrypoints = [
  "build-resources/c420ui/scripts/build-runtime.ts",
  "build-resources/c420ui/scripts/build-preload-bundle.ts",
  "build-resources/c420ui/scripts/copy-runtime-assets.ts",
  "build-resources/c420ui/scripts/clean-runtime-build.ts",
...
function validateNoCoreProductDetectionLogic(
  ...
  const scriptsCoreDir = path.join(rootDir, "build-resources/canva-linux/checks/core");
  ...
  for (const entry of fs.readdirSync(scriptsCoreDir, { withFileTypes: true })) {
    const relativePath = `build-resources/canva-linux/checks/core/${entry.name}`;
    if (!entry.isFile() || !entry.name.endsWith(".ts")) {
      failures.push(`${relativePath}: build-resources/canva-linux/checks/core is infrastructure-check-only and must not contain runtime/product entrypoints`);
      continue;
    }
    if (!allowedCoreFiles.has(entry.name)) {
      failures.push(`${relativePath}: build-resources/canva-linux/checks/core is infrastructure-check-only; move runtime/product code to its owning domain`);
    }
...
function validateDependentProjectBoundaryLayout(
  ...
  const requiredPaths = [
    ...
    "build-resources/canva-linux/actions",
  ] as const;
  ...
  const removedPaths = [
    ...
    "build-resources/canva-linux/checks/core/action-runner.ts",
    "build-resources/canva-linux/checks/core/overview-status.ts",
    "build-resources/canva-linux/checks/core/action-registry.ts",
    "build-resources/canva-linux/checks/core/validate-actions.ts",
...
function validateC420UIOperationalOwnership(
  ...
  const forbiddenCanvaLinuxOperationalPaths = [
    ...
    "build-resources/canva-linux/packaging/flatpak/build.ts",
    "build-resources/canva-linux/packaging/flatpak/runtime.ts",
    "build-resources/canva-linux/packaging/flatpak/scope.ts",
  ] as const;
...
function checkNoLegacyActionRunner(rootDir: string, failures: string[]): void {
  ...
  const removedFiles = [
    `build-resources/canva-linux/checks/core/${legacyActionRunnerStem}.ts`,
    `build-resources/canva-linux/checks/core/${legacyCompatibilityStem}.ts`,
...
function main(): number {
  ...
  const adapterDir = path.join(rootDir, "build-resources/canva-linux/c420ui-adapter");
  ...
  for (const file of allRepositoryFiles(rootDir).filter((entry) => entry.startsWith("build-resources/canva-linux/c420ui-adapter/"))) {
    const content = fs.readFileSync(path.join(rootDir, file), "utf8");
