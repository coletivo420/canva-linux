...
test("runtime.ts does not expose CANVA_DISABLE_GPU fallback", () => {
  const source = fs.readFileSync(path.join(repoRoot, "build-resources/electron/main/runtime.ts"), "utf8");
...
test("main runtime opens Canva, not the project website, as the app home URL", () => {
  const mainSource = fs.readFileSync(
    path.join(repoRoot, "build-resources/electron/main/index.ts"),
    "utf8",
  );
