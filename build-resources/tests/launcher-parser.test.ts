...
test("primary builder wrapper prefers c420ui-builder bootstrap bundle before build fallback", () => {
  ...
  assert.doesNotMatch(wrapper, /canva-linux-c420ui-builder\.mjs/);
  assert.doesNotMatch(wrapper, /canva-linux-c420ui-builder\.js/);
  assert.ok(
    wrapper.indexOf("build-resources/c420ui/bootstrap/generated/c420ui-builder.mjs") <
...
test("builder source delegates direct actions through c420ui CLI bridge", () => {
  ...
  assert.match(source, /build-resources\/c420ui\/bootstrap\/generated\/run-c420ui-cli\.mjs/);
  assert.match(source, /\.build\/scripts\/run-c420ui-cli\.mjs/);
  assert.match(source, /No direct action was provided/);
});

test("builder source opens c420ui UI by default", () => {
  const source = fs.readFileSync("build-resources/c420ui/scripts/c420ui-builder.ts", "utf8");
  assert.match(source, /build-resources\/c420ui\/bootstrap\/generated\/run-c420ui\.mjs/);
  assert.match(source, /\.build\/scripts\/run-c420ui\.mjs/);
