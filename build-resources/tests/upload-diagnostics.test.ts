...
test("describes file inputs", () => {
  ...
  globalThis.HTMLInputElement =
    /** @type {typeof HTMLInputElement} */ TestHTMLInputElement;
  try {
    assert.deepEqual(
      describeFileInput(
        /** @type {HTMLInputElement} */ /** @type {unknown} */ new TestHTMLInputElement(),
      ),
      {
        accept: "image/*",
        multiple: "true",
    ...
    assert.equal(
      describeFileInput(/** @type {EventTarget} */ /** @type {unknown} */ {}),
      null,
    );
  } finally {
...
test("formats and summarizes files", () => {
  assert.equal(
    formatFileDescriptor({ name: "image.png", type: "image/png", size: 42 }),
...
test("remembers and summarizes recent upload ingress", () => {
  const scope =
    /** @type {typeof globalThis & { __canvaUploadIngressCounter?: number, __canvaLastUploadIngress?: unknown }} */ globalThis;
  ...
  try {
    ...
    assert.match(
      recentUploadIngressSummary(),
      /^id=1 source=drop ageMs=\d+ files=2 types=Files,text\/plain div#dropzone$/,
    );
  } finally {
