...
rm -f "${tmp}"
if [[ "${NATIVE_SCOPE}" == "system" ]]; then
  install_icons "${NATIVE_SCOPE}" "${REPO_ROOT}/build-resources/canva-linux/assets/icons/hicolor" "/usr/local/share/icons/hicolor"
else
  install_icons "${NATIVE_SCOPE}" "${REPO_ROOT}/build-resources/canva-linux/assets/icons/hicolor" "${HOME}/.local/share/icons/hicolor"
fi
