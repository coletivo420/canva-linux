...
test("primary builder wrapper prefers c420ui-builder bootstrap bundle before build fallback", () => {
  ...
  assert.ok(
    wrapper.indexOf("packages/c420ui/bootstrap/generated/c420ui-builder.cjs") <
      wrapper.indexOf(".build/packages/c420ui/scripts/c420ui-builder.js"),
  );
});

test("builder source delegates direct actions through c420ui CLI bridge", () => {
  const source = fs.readFileSync("packages/c420ui/scripts/c420ui-builder.ts", "utf8");
  assert.doesNotMatch(source, /DIRECT_ACTION_FLAGS/);
  assert.match(source, /hasBridgeAction/);
  assert.match(source, /packages\/c420ui\/bootstrap\/generated\/run-c420ui-cli\.cjs/);
  assert.match(source, /\.build\/packages\/c420ui\/scripts\/run-c420ui-cli\.js/);
  assert.match(source, /No direct action was provided/);
});

test("builder source opens c420ui UI by default", () => {
  const source = fs.readFileSync("packages/c420ui/scripts/c420ui-builder.ts", "utf8");
  assert.match(source, /packages\/c420ui\/bootstrap\/generated\/run-c420ui\.cjs/);
  assert.match(source, /\.build\/packages\/c420ui\/scripts\/run-c420ui\.js/);
