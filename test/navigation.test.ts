...
test('detects OAuth provider URLs', () => {
  assert.equal(isOAuthProviderUrl('https://accounts.google.com/o/oauth2/v2/auth'), true);
  assert.equal(isOAuthProviderUrl('https://login.microsoftonline.com/common/oauth2'), true);
  assert.equal(isOAuthProviderUrl('https://example.com/login'), false);
});

test('detects Canva OAuth callbacks', () => {
  assert.equal(detectCanvaOAuthCallback('https://www.canva.com/oauth/authorized/foo'), 'authorized');
  assert.equal(detectCanvaOAuthCallback('https://www.canva.com/oauth/foo'), 'oauth');
...
test('extracts hostnames safely', () => {
  assert.equal(extractHostname('https://www.canva.com/design'), 'www.canva.com');
