...
function createFakeDom(context) {
...
test('removeClEyeDropperUi removes the LTCode-compatible overlay id', () => {
  ...
  try {
    ...
    const { removeClEyeDropperUi } = loadRuntimeModule('preload/cl-eyedropper/index');
...
test('CLEyeDropper preserves the LTCode canvas event model and scaled picking', async () => {
  ...
  /** @type {any} */ (globalThis).requestAnimationFrame = fakeWindow.requestAnimationFrame;

  try {
    const { CLEyeDropper, installClEyeDropperScalingPatch } = loadRuntimeModule('preload/cl-eyedropper/index');
    ...
    assert.deepEqual(await resultPromise, { hex: '#ff0080', rgb: [255, 0, 128] });
    assert.deepEqual(logs[0], ['eyedropper:library', 'picked', '#ff0080', 100, 50, '[255,0,128]']);
    assert.equal(body.children.length, 0);
