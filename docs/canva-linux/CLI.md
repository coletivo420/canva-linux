...
The launcher opens c420ui without arguments. With a direct action flag, the launcher invokes `packages/c420ui/bootstrap/generated/run-c420ui-cli.cjs` when the generated bundle is present, with `.build/packages/c420ui/scripts/run-c420ui-cli.js` kept only as a development fallback. Direct actions resolve through c420ui and the Canva Linux action registry.
...
- `./canva-linux-c420ui-builder` starts `packages/c420ui/bootstrap/generated/run-c420ui.cjs` when present.
- Direct actions such as `./canva-linux-c420ui-builder --doctor --dry-run` start `packages/c420ui/bootstrap/generated/run-c420ui-cli.cjs` when present.
