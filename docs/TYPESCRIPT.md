...
Canva Linux is TypeScript-first for Electron runtime code and Node.js maintenance logic.
...
- The TypeScript migration checklist for the `0.1.4.11-dev.57` line is closed for `scripts/core`, top-level `scripts/*.js` wrappers, build/TUI wrappers, and Flathub npm source generation.
...
- Node.js maintenance script entrypoints remain as compatibility wrappers where needed; real build, runtime, TUI, test-runner, Flathub source-generation and core validation logic lives in TypeScript.
...
- `check-typescript-first.ts` enforces TypeScript-first script guardrails, minimal `scripts/check-*.js` wrappers, package-script core entry coverage, Flathub source generator coverage, and documentation coverage for migrated core entries.
- All core validations are integrated into `npm run check:scripts-core`.
- Build/TUI wrappers (`build-runtime`, `build-preload-bundle`, `copy-runtime-assets`, `clean-runtime-build`, `electron-builder-before-build`, `run-node-tests`, and `run-tui`) are enforced so the `.js` files stay as compatibility entrypoints and the `.ts` files remain the real implementations.
...
- `run-node-tests.js` -> `run-node-tests.ts`
...
- `packaging/flathub/scripts/generate-npm-sources.js` is a thin compatibility wrapper through the shared TypeScript script runner.
- `packaging/flathub/scripts/generate-npm-sources.sh` invokes the TypeScript-backed `.js` wrapper so existing Flathub workflows use the migrated generator.
