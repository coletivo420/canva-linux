...
- Confirm root execution is blocked with a clear message before the TUI or any direct CLI action starts.
...
- Confirm terminal text selection mode disables TUI mouse handling globally on the next TUI start while keyboard scroll and F5 log copy continue to work.
