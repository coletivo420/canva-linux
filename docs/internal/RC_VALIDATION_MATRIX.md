...
| Command | Status | Expected result | Failure meaning | Owner domain |
| --- | --- | --- | --- | --- |
| `node -p "require('./build-resources/c420ui/bootstrap/generated/manifest.json').c420uiVersion" && node -p "require('./build-resources/c420ui/bootstrap/generated/manifest.json').dependentProjectVersion"` | Required for bootstrap RC validation | Bootstrap identity reports c420ui engine version from `build-resources/c420ui/package.json` and dependent-project version from the root `package.json`. | The bootstrap manifest may have collapsed engine and dependent-project identity into one ambiguous version. | c420ui bootstrap |
| `rm -rf node_modules .build && ./canva-linux-c420ui-builder --doctor --dry-run` | Required for bootstrap RC validation; root containers may record blocked | A clean checkout starts direct c420ui CLI through `build-resources/c420ui/bootstrap/generated/run-c420ui-cli.mjs` without a missing `esbuild` failure or launcher-side npm install. | Stage 0 bootstrap or Stage 1 dependency ownership may be broken. | c420ui bootstrap |
| `rm -rf node_modules .build && ./canva-linux-c420ui-builder` | Required for bootstrap RC validation; root containers may record blocked | A clean checkout starts interactive c420ui through `build-resources/c420ui/bootstrap/generated/run-c420ui.mjs` before dependent-project dependency repair appears in the UI logs. | The bootstrap may still block on dependency repair before the UI is mounted. | c420ui bootstrap |
...
| --- | --- | --- |
| Bootstrap bundle exists | Pass | `build-resources/c420ui/bootstrap/generated/run-c420ui.mjs` and `build-resources/c420ui/bootstrap/generated/run-c420ui-cli.mjs` are committed bootstrap entrypoints; `node build-resources/c420ui/bootstrap/generated/run-c420ui-cli.mjs --help` passed after `rm -rf node_modules .build`. |
| c420ui version is independent | Pass | `build-resources/c420ui/bootstrap/generated/manifest.json` records `c420uiVersion` as `0.1.0`. |
| Dependent project version is separate | Pass | `build-resources/c420ui/bootstrap/generated/manifest.json` records `dependentProjectVersion` as `0.1.4-15.Dev.9`. |
...
| Stale hash detection works | Pass | After `printf '\n' >> build-resources/c420ui/scripts/build-bootstrap.ts`, `npm run check:c420ui-bootstrap` failed with `build-resources/c420ui/bootstrap/generated/manifest.json: sourceHash is stale; run npm run build:c420ui-bootstrap`. |
| Launcher does not install npm deps | Pass | `rg -n "npm (install\|ci)\|npm\\s+install\|npm\\s+ci" canva-linux-c420ui-builder` returned no matches; `canva-linux-c420ui-builder` resolves `build-resources/c420ui/bootstrap/generated/run-c420ui-cli.mjs` before the `.build` fallback. |
| Clean checkout starts without esbuild | Blocked: environment limitation | After `rm -rf node_modules .build`, `node -e "require.resolve('esbuild')"` reported `esbuild not resolvable`; `node build-resources/c420ui/bootstrap/generated/run-c420ui-cli.mjs --help` and `node build-resources/c420ui/bootstrap/generated/run-c420ui-cli.mjs --doctor --dry-run` passed, but `./canva-linux-c420ui-builder --help`, `./canva-linux-c420ui-builder --doctor --dry-run`, and `./canva-linux-c420ui-builder` exited at the root-user guard before launcher dispatch. |
...
from `build-resources/c420ui/package.json`, while `dependentProjectVersion` is sourced from the repository root `package.json`.
...
startup task, not as pre-UI logic in `build-resources/c420ui/scripts/run-c420ui.ts`.
