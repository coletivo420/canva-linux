...
RC validation is blocked when `bootstrap/c420ui/manifest.json` `sourceHash` does not match the current bootstrap source-hash inputs. Rebuild with `npm run build:c420ui-bootstrap`, then run `npm run check:c420ui-bootstrap`; the check must pass without requiring additional generated-file changes.
