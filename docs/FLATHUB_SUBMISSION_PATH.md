...
1. **Local development flow**: `./canva-linux.sh --install-flatpak` builds as the current user, exports `repo/`, and installs from that local repo into the system Flatpak scope by default.
2. **GitHub release flow**: `./canva-linux.sh --bundle-flatpak` exports `repo/` and then creates `.flatpak` via `flatpak build-bundle`.
