...
repository-check boundary. Transitional shell fallbacks must not be restored.
...
- `scripts/preflight-common.sh` is repository-check-only. It may expose shared
  ...
  `validate_package_scripts`, `detect_package_version`, and
  `validate_package_version_semver`.

It must not own npm install policy, dependency repair policy, or transitional
bootstrap fallbacks.
