...
- Submission workflow (new path): `build-resources/canva-linux/packaging/flathub/manifest.yml` + generated npm source manifest + submission scripts.
...
3. **Flathub submission flow**: uses `build-resources/canva-linux/packaging/flathub/manifest.yml` + generated sources + submission validation; `.flatpak` bundle generation is not part of the submission path.
