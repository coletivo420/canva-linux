...
The Canva editor runtime uses `packages/electron/preload/canva.bundle.js`, but that file is generated from the modular source files before the Electron build.

For source review, treat the maintained files under `packages/electron/preload/*.ts`, `packages/electron/shared/*.ts`, and `scripts/build-preload-bundle.ts` as the source of truth. The final Flathub build path should regenerate the bundle from those sources instead of relying on a local untracked bundle artifact.
