...
const AUTH_PATH_RE =
  /\/(?:login|signup|register|oauth|sso|auth|signin|account)(?:[/?#]|$)/i;
const CANVA_AUTH_HINT_RE =
  /(?:google|facebook|apple|microsoft|oauth|sso|signup|login|continue)/i;
...
function isCanvaUrl(url: string): boolean {
  try {
    ...
    return (
      parsed.protocol === "https:" && INTERNAL_HOST_RE.test(parsed.hostname)
    );
  } catch {
    return false;
  }
...
function isOAuthProviderUrl(url: string): boolean {
  try {
    ...
    return (
      ...
      OAUTH_PROVIDER_HOSTS.some((re) => re.test(parsed.hostname))
    );
  } catch {
    return false;
  }
...
function shouldGrantRemotePermission(
  permission: string,
  origin: string,
  details: PermissionDetails = {},
): boolean {
  const canvaTrusted =
    isCanvaUrl(origin) || isCanvaUrl(details.requestingUrl || "");
...
function shouldTreatAsOauthPopup({
  ...
  if (
    isBlankPopupUrl(url) &&
    frameName &&
    /auth|oauth|login|signin|account|popup/i.test(frameName)
  )
    return true;
  if (
    (disposition === "new-window" || disposition === "foreground-tab") &&
    isCanvaAuthUrl(openerUrl)
  )
    return true;
  return false;
}
