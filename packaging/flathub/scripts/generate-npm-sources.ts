...
const supportedIntegrityAlgorithms = [
  "sha512",
  "sha384",
  "sha256",
  "sha1",
...
type SupportedIntegrityAlgorithm =
  (typeof supportedIntegrityAlgorithms)[number];
...
function readLockfile(): PackageLock {
  if (!fs.existsSync(lockfilePath))
    fail(`missing package lock: ${path.relative(repoRoot, lockfilePath)}`);
...
function validateRemoteTarball(packagePath: string, resolved: string): URL {
  if (isLocalOrForbiddenResolved(resolved)) {
    fail(
      `${packagePath}: local, workspace, link, or node_modules resolved paths are not allowed in Flathub npm sources (${resolved})`,
    );
  }
  ...
  if (url.protocol !== "https:") {
    fail(
      `${packagePath}: resolved URL must use https for Flathub npm sources (${resolved})`,
    );
  }
  if (!url.pathname.endsWith(".tgz")) {
    fail(
      `${packagePath}: resolved URL must point at an npm tarball (${resolved})`,
    );
  }
...
function convertIntegrity(
  integrityValue: string,
  packagePath: string,
): { algorithm: SupportedIntegrityAlgorithm; digestHex: string } {
  ...
  fail(
    `${packagePath}: integrity must include one of ${supportedIntegrityAlgorithms.join(", ")}`,
  );
}
...
function destinationFilename(candidate: SourceCandidate): string {
  ...
  const packageName = sanitizePackageName(
    candidate.name || packageNameFromPath(candidate.packagePath),
  );
  const destination =
    packageName && !baseName.startsWith(packageName)
  ...
  if (
    ...
    fail(
      `${candidate.packagePath}: unsafe generated destination filename (${destination})`,
    );
  }
...
function candidateFromPackage(
  packagePath: string,
  meta: LockfilePackage,
): SourceCandidate | null {
  if (packagePath === "") return null;
  if (!packagePath.startsWith("node_modules/")) {
    fail(
      `${packagePath}: package-lock package entries must be rooted under node_modules`,
    );
  }
  ...
  if (!meta.resolved || !meta.integrity)
    fail(
      `${packagePath}: package-lock package entry must include both resolved and integrity`,
    );
...
function collectDependencyCandidates(
  dependencies: Record<string, LockfileDependency | null> | undefined,
  parentPath = "dependencies",
): SourceCandidate[] {
  if (!dependencies) return [];

  const candidates: SourceCandidate[] = [];
  for (const [name, meta] of Object.entries(dependencies).sort(
    ...
    if (meta.resolved || meta.integrity) {
      if (!meta.resolved || !meta.integrity)
        fail(
          `${packagePath}: dependency entry must include both resolved and integrity`,
        );
      candidates.push({
        packagePath,
        name,
    ...
    candidates.push(
      ...collectDependencyCandidates(meta.dependencies, packagePath),
    );
  }
  return candidates;
}
...
function sourceFromCandidate(candidate: SourceCandidate): NpmSource {
  ...
  const convertedIntegrity = convertIntegrity(
    candidate.integrity,
...
function collectSources(lockfile: PackageLock): NpmSource[] {
  ...
  const fallbackCandidates =
    candidates.length > 0
      ? []
      : collectDependencyCandidates(lockfile.dependencies);
  ...
  for (const candidate of [...candidates, ...fallbackCandidates]) {
    ...
    if (existing) {
      const existingHashKey = supportedIntegrityAlgorithms.find(
        (algorithm) => existing[algorithm],
      );
      const sourceHashKey = supportedIntegrityAlgorithms.find(
        (algorithm) => source[algorithm],
      );
      if (
        existing["dest-filename"] !== source["dest-filename"] ||
        !existingHashKey ||
        existingHashKey !== sourceHashKey ||
        existing[existingHashKey] !== source[sourceHashKey]
      ) {
        fail(
          `${candidate.packagePath}: duplicate URL has conflicting filename or integrity (${source.url})`,
        );
      }
      continue;
    }
  ...
  const sources = [...byUrl.values()].sort((left, right) => {
    ...
    return byUrlCompare !== 0
      ? byUrlCompare
      : left["dest-filename"].localeCompare(right["dest-filename"]);
  });

  if (sources.length === 0)
    fail("no remote npm tarball sources found in package-lock.json");
  return sources;
}

function createGeneratedSourcesModule(
  sources: NpmSource[],
): GeneratedSourcesModule {
  return {
    name: "generated-npm-sources",
    buildsystem: "simple",
    "build-commands": [
...
export function main(): void {
  ...
  console.log(
    `Generated ${path.relative(repoRoot, outputPath)} with ${sources.length} sources.`,
  );
}
