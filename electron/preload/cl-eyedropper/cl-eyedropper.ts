...
export class CLEyeDropper {
  ...
  open(canvasOrContext: CLEyeDropperOpenTarget, options: CLEyeDropperOptions = {}): Promise<LTCodeCompatibleResult> {
  ...
  _createUI(canvasOrContext: CLEyeDropperOpenTarget): void {
    ...
    this._canvas.addEventListener('mousemove', this._onMouseMoveBound, { passive: true });
    this._canvas.addEventListener('mouseleave', this._onMouseLeaveBound, { passive: true });
    this._canvas.addEventListener('mouseenter', this._onMouseEnterBound, { passive: true });
  ...
  _onMouseMove(event: MouseEvent): void {
    if (!this._canvas || !this._canvasCache || !this._previewColor || !this._colorDisplay || !this._preview) {
      return;
    }
  ...
  _updateMagnifier(x: number, y: number, event: MouseEvent, rgb: RgbTuple, hex: string): void {
  ...
  _onMouseLeave(): void {
    if (!this._canvas || !this._magnifier || !this._preview || !this._canvasCache) return;
  ...
  _removeUI(): void {
    ...
    if (this._canvas) {
      this._canvas.style.cursor = 'default';
      if (this._onMouseMoveBound) this._canvas.removeEventListener('mousemove', this._onMouseMoveBound);
      if (this._onMouseLeaveBound) this._canvas.removeEventListener('mouseleave', this._onMouseLeaveBound);
      if (this._onMouseEnterBound) this._canvas.removeEventListener('mouseenter', this._onMouseEnterBound);
      if (this._onClickBound) this._canvas.removeEventListener('click', this._onClickBound);
    }
...
function getScaledCanvasPosition(instance: CLEyeDropper, event: MouseEvent): { x: number; y: number; rect: DOMRect | null } {
...
export function installClEyeDropperScalingPatch(log: EyeDropperLog): void {
  const patchedClass = CLEyeDropper as typeof CLEyeDropper & { __canvaScalingPatchInstalled?: boolean };
  ...
  proto._onMouseMove = function onMouseMovePatched(this: CLEyeDropper, event: MouseEvent): void {
  ...
  proto._onClick = function onClickPatched(this: CLEyeDropper, event: MouseEvent): void {
